...
enabled_providers = ENV.fetch("VAGRANT_SPEC_PROVIDERS", "vmware_desktop").split(",")
    ...
    enabled_providers.each do |provider_name|
      global_config.vm.define("#{box_name.split('/').last}-#{provider_name}") do |config|
        ...
        config.vm.provision :shell,
          path: "./scripts/#{platform}-setup.#{provider_name}.sh",
          run: "once",
          env: {
            "VAGRANT_VMWARE_LICENSE_URL" => ENV["VAGRANT_VMWARE_LICENSE_URL"],
        ...
        guest_boxes.each_with_index do |box_info, idx|
          ...
          config.vm.provision(
            ...
            env: {
              ...
              "VAGRANT_SPEC_BOX" => "/vagrant/spec/vagrant-spec/boxes/#{guest_box.sub('/', '_')}.#{provider_name}.#{box_version}.box"
            }
          )
        end
      end
